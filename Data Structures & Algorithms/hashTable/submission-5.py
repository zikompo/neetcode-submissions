...
def q_hash(key, capacity, i):
        return (primary_hash(key, capacity) + i**2)%capacity
...
class HashTable:
    ...
    def insert(self, key: int, value: int) -> None:
        ...
        if self.hash[index_to_insert] == None:
            ...
            i = 1
            while self.hash[q_hash(key, self.capacity, i)] != None:
                x = self.hash[q_hash(key, self.capacity, i)]
            ...
            self.hash[q_hash(key, self.capacity, i)] = Node(key, value)
            self.size += 1
            self.check_load_factor()
    ...
    def remove(self, key: int) -> bool:
        ...
        if self.hash[index_to_check] != None and self.hash[index_to_check].key == key:
            ...
            while self.hash[q_hash(key, self.capacity, i)] != None:
                ...
                x = self.hash[q_hash(key, self.capacity, i)]
                if x.key == key:
                    self.hash[q_hash(key, self.capacity, i)] = None
                    self.size-=1
                    return True
                i += 1
            return False
